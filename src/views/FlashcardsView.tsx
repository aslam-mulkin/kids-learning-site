...
export default function FlashcardsView({ cards }: Props) {
  ...
  // Reset when incoming cards change to avoid stale state
  useEffect(() => {
    setQueue(initial)
    setIdx(0)
    setShowAns(false)
  ...
  function mark(correct: boolean) {
    ...
    setQueue(prev => {
      ...
      if (!correct) {
        ...
        const pos = Math.floor(Math.random() * (copy.length - idx + 1)) + idx // between idx..end
        copy.splice(pos, 0, current)
        // stay at same idx to show the card that moved into this slot
